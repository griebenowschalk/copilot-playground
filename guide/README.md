...
| `skills/figma-to-code/` | Copyable Figma design-to-code skill template (`SKILL.md`) referenced from Step 6 §6.7 — install in `.claude/skills/figma-to-code/` when Figma is opted in, alongside a `README.md` documenting `FIGMA_API_KEY` setup. Visible to Copilot Chat directly — no separate prompt file |
...
| `step-6-skills.md` | Project skills (`.claude/skills/<name>/SKILL.md`) — shared with Copilot Chat directly, no `.github/prompts/` |
...
| 5 | MCP | `step-5-mcp.md` | `.mcp.json` + `.vscode/mcp.json`, `CLAUDE.md` MCP section |
| 6 | Skills | `step-6-skills.md` | `.claude/skills/<name>/SKILL.md` (staple + codebase-specific; `figma-to-code` + `README.md` if Figma) |
...
It removes only the named harness artifacts (`AGENTS.md`, `CLAUDE.md`, `.mcp.json`, `.github/`, `.vscode/`, `.claude/`, `docs/`, `graphify-out/`, `.graphifyignore`, `scripts/graphify-setup.sh`, the per-run `guide/.harness-progress.md` ledger) and restores `.gitignore` from git if the target is a work tree. **App-owned files the harness only partially edits are stripped in place, never deleted or reverted wholesale:** the root `README.md` loses its "Codebase graph (Graphify — optional)" section, and `package.json` loses its `graphify:setup` script entry. **Application code is otherwise never touched**, including any `.env` / `.env.example` you manage yourself.
